...
state.drawingIntel = { tolerances: [], threads: [], chamfers_detected: [], requires_chamfer_decision: true, notes: [] };
state.threadCatalog = [];
...
function buildEngineeringContext() {
  const tolerances = state.drawingIntel.tolerances?.join('; ') || 'не распознаны';
...
function collectProjectData(){return{contourPoints:state.contourPoints,editor:collectEditorSettings(),shopturn:collectShopTurnData(),operationRoute:state.operationRoute,projectThreads:state.projectThreads,chamfers:state.chamfers,drawingIntel:state.drawingIntel,stockMode:state.stockMode,blank:{diameter:$('blankDiameter').value,length:$('blankLength').value,width:$('blankWidth').value,height:$('blankHeight').value,millLength:$('blankLengthMill').value},zeroReference:$('zeroReference').value,firstSide:$('firstSide').value,notes:$('stockNotes').value,fileName:state.file?.name||null,updatedAt:Date.now()};}
function applyProjectData(d={}){state.contourPoints=Array.isArray(d.contourPoints)?clone(d.contourPoints):[];applyEditorSettings(d.editor||{});state.stockMode=d.stockMode||'lathe';document.querySelectorAll('.segmented-item').forEach(b=>b.classList.toggle('active',b.dataset.mode===state.stockMode));$('latheFields').classList.toggle('hidden',state.stockMode!=='lathe');$('millFields').classList.toggle('hidden',state.stockMode!=='mill');const b=d.blank||{};$('blankDiameter').value=b.diameter||'';$('blankLength').value=b.length||'';$('blankWidth').value=b.width||'';$('blankHeight').value=b.height||'';$('blankLengthMill').value=b.millLength||'';$('zeroReference').value=d.zeroReference||'';$('firstSide').value=d.firstSide||'';$('stockNotes').value=d.notes||'';applyShopTurnData(d.shopturn||{});state.operationRoute=Array.isArray(d.operationRoute)?clone(d.operationRoute):[];state.projectThreads=Array.isArray(d.projectThreads)?clone(d.projectThreads):[];state.chamfers=Array.isArray(d.chamfers)?clone(d.chamfers):[];state.drawingIntel=d.drawingIntel||{tolerances:[],threads:[],chamfers_detected:[],requires_chamfer_decision:true,notes:[]};if((state.drawingIntel.tolerances?.length||state.drawingIntel.threads?.length||state.chamfers.length||state.projectThreads.length))$('drawingIntelligencePanel').classList.remove('hidden');state.selectedIndex=0;state.activeRouteIndex=-1;state.validation=[];renderDrawingIntelligence();renderProjectThreads();renderChamferEditor();renderOperationRoute();renderEditor();syncFileUi();}
...
function resetWorkspaceForNewProject(){
  state.currentProjectId=null;state.currentProjectName='Локальный черновик';state.file=null;state.image=null;state.crop=null;state.contourPoints=[];state.selectedIndex=0;state.undoStack=[];state.redoStack=[];state.validation=[];state.operationRoute=[];state.activeRouteIndex=-1;state.projectThreads=[];state.chamfers=[];state.drawingIntel={tolerances:[],threads:[],chamfers_detected:[],requires_chamfer_decision:true,notes:[]};
  ...
  localStorage.removeItem('personal-ai-pro-draft');updateProjectUi();syncFileUi();renderDrawingIntelligence();renderProjectThreads();renderChamferEditor();renderOperationRoute();renderEditor();renderShopTurn();scheduleAutosave();
}
...
function renderDrawingIntelligence(){
  ...
  const t=state.drawingIntel.tolerances||[],threads=state.drawingIntel.threads||[];
  $('toleranceList').innerHTML=t.length?t.map(x=>`<div class="intel-token tolerance-token">${escapeHtml(x)}</div>`).join(''):'<span class="muted-text">Явные допуски не распознаны. Проверь основную надпись и технические требования.</span>';
  ...
  $('drawingIntelBadge').textContent=`Допуски: ${t.length} · Резьбы: ${threads.length}`;
...
initThemeControls();updateDeviceModeLabel();initShopTurn();loadHealth();loadThreadCatalog();loadLocalDraft();updateProjectUi();syncFileUi();renderDrawingIntelligence();renderProjectThreads();renderChamferEditor();renderOperationRoute();renderEditor();renderShopTurn();
