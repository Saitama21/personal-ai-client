...
export function validateDrilling(input, feature) {
  ...
  if (feature.startZ > EPS || feature.startZ < -input.blankLength - EPS) errors.push(issue('DRILL_START_Z_INVALID', 'Начало осевого отверстия должно находиться в диапазоне детали.', 'error', 'features'));
  if (feature.depth > input.blankLength + feature.startZ + EPS) errors.push(issue('DRILL_DEPTH_OUTSIDE_PART', 'Глубина осевого отверстия выходит за левый торец детали.', 'error', 'features'));
  const localDiameter = diameterAtZ(input.contour, feature.startZ - Math.min(feature.depth, 0.5));
...
export function planDrilling(input, feature) {
  ...
  const safe = { x: clearDiameter, z: feature.retractZ };
  const axisSafe = { x: 0, z: feature.retractZ };
  ...
  while (drilled < feature.depth - EPS) {
    ...
    const startZ = feature.startZ - drilled + (drilled > 0 ? feature.chipClearance : 0);
    const endZ = feature.startZ - nextDepth;
  ...
  return {
    ...
    operations, moves, feature, hole: { type: 'blind_axial', diameter: feature.diameter, depth: feature.depth, startZ: feature.startZ, tipAngle: feature.tipAngle },
