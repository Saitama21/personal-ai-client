...
def test_health_reports_shopturn_feature():
    ...
    assert body["version"] == "2.8.1 Universal Responsive"
...
def test_index_disables_browser_cache():
    ...
    assert response.headers.get("x-app-version") == "2.8.1 Universal Responsive"


def test_static_assets_require_revalidation():
    response = client.get("/static/app.js?v=2.7.9-native-ipad-upload")
    assert response.status_code == 200
