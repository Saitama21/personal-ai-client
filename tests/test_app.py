...
def test_health_reports_shopturn_feature():
    ...
    assert body["version"] == "2.7.4-ipad-layout-activation-fix"
...
def test_index_disables_browser_cache():
    ...
    assert response.headers.get("x-app-version") == "2.7.4-ipad-layout-activation-fix"


def test_static_assets_require_revalidation():
    response = client.get("/static/app.js?v=2.7.4-ipad-layout-activation-fix")
    assert response.status_code == 200
